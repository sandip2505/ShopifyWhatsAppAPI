...
upsellController.addProduct = async (req, res) => {
  const { shop_name, products, autoRecommendUpsell, upsellLocation, customSelector } = req.body; // Expecting all relevant fields
  ...
  if (autoRecommendUpsell === false && (!Array.isArray(products) || products.length === 0)) {
    return res.status(400).json({ 
      message: 'Invalid input. Products are required when autoRecommendUpsell is disabled.' 
    });
  }
  ...
  try {
    ...
    if (existingShop) {
      if (autoRecommendUpsell) {
        ...
        existingShop.customSelector = upsellLocation === "custom" ? customSelector || "" : ""; // Only set customSelector if upsellLocation is "custom"
      }
      else {
      ...
      const newShop = new upsell({
        shop_name,
        autoRecommendUpsell,
        products: autoRecommendUpsell ? [] : products.filter(product => product), // Only add products if autoRecommendUpsell is disabled
        upsellLocation: upsellLocation || "default", // Default location if not provided
...
upsellController.updateTitle = async (req, res) => {
  try {
    const { shop_name, title, buttonLabel } = req.body; // Extract the relevant fields from the request body
    ...
    if (shop) {
      ...
      const newShop = new upsell({ // Replace 'Shop' with your model's name
        shop_name: shop_name,
        title: title,
      ...
      return res.status(201).json({ message: 'New shop added successfully', newShop });
    }
  } catch (error) {
    console.error('Error saving/updating shop:', error);
    return res.status(500).json({ message: 'Error saving/updating shop.', error });
  }
};
